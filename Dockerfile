...
# Inject debugging into rcutils CMake - capture to memory before, print after
RUN cat > /tmp/inject_debug.sh << 'SCRIPTEOF'
#!/bin/bash
TARGET=/opt/ros/humble/share/rcutils/cmake/ament_cmake_export_libraries-extras.cmake

# Create the pre-capture code
cat > /tmp/debug_inject.txt << 'CMAKEOF'
    # DEBUG: Capture ALL CMake variables to memory BEFORE find_library
...
# Create the post-print code
cat > /tmp/debug_after.txt << 'CMAKEOF'
      # DEBUG: Print captured state AFTER find_library completed
      message(STATUS "DEBUG rcutils: ========== STATE CAPTURED BEFORE FIND_LIBRARY ==========")
